...
struct SegTree
{
	...
	void propagate(int node, int L, int R)
	{
		...
		if(lazy[node]%2==0 && a[L]%2!=0)
		{
			st[node].odd--;
			st[node].even++;
		}
		else if(lazy[node]%2!=0 && a[L]%2==0)
		{
			st[node].odd++;
			st[node].even--;
		}
...
int main(int argc, char const *argv[])
{
	...
	while(q--)
	{
		...
		{
			cout << seg.query(u,v).even << endl;
		}
		else
		{
			cout << seg.query(u,v).odd << endl;
		}
	}
	return 0;
}
...
//     if(start == end)
//     {
//     	if(a[start]&1)seg[node].odd = 1;
//         else seg[node].even = 1;
//         return ;
//     }
...
//     seg[node].even = seg[node<<1].even + seg[node<<1|1].even;//seg[node] = seg[2*node] + seg[2*node+1];
//     seg[node].odd = seg[node<<1].odd + seg[node<<1|1].odd;
// }
...
// 		{
// 			u--, v--;
// 			cout << query(1,0,n-1,u,v).even << endl;
// 		}
// 		else
// 		{
// 			u--, v--;
// 			cout << query(1,0,n-1,u,v).odd << endl;
// 		}
// 	}
// 	return 0;
// }